...
# # ─────────────────────────────────────────────────────────────────────────────
# # Trigger Point CSV Logging Helpers
# # ─────────────────────────────────────────────────────────────────────────────

# TRIGGER_CSV_HEADER = ["User_ID", "Timestamp", "HR_Identification", "AI_Warning"]

# def ensure_trigger_csv_header(csv_path) -> None:
#     """Create the trigger-point CSV with its header row if it doesn't exist yet."""
#     csv_path = Path(csv_path)
#     if not csv_path.exists():
#         csv_path.parent.mkdir(parents=True, exist_ok=True)
#         with open(csv_path, "w", newline="", encoding="utf-8") as f:
...
# def write_trigger_row(csv_path, user_id: str, timestamp_sec: str,
#                       hr_text: str, ai_warning: str) -> None:
#     """Append one evidence row to the trigger-point CSV."""
#     csv_path = Path(csv_path)
#     ensure_trigger_csv_header(csv_path)
#     with open(csv_path, "a", newline="", encoding="utf-8") as f:
#         writer = csv.writer(f)
...
#     # 1b. Unique ID & CSV path including Name and Unique ID
#     user_id = f"USER_{uuid.uuid4().hex[:6].upper()}"
#     trigger_csv_path = video_dir / f"trigger_points_{safe_name}_{user_id}.csv"
#     ensure_trigger_csv_header(trigger_csv_path)
    
#     # Register session start in CSV (Timestamp in seconds)
#     write_trigger_row(
#         trigger_csv_path,
#         user_id,
#         "0.00s",
...
#     # Gaze / Iris settings
#     EYE_OPEN_FOR_GAZE_EAR = 0.23   # must be clearly open (prevents blink false positives)
#     IRIS_STREAK_TRIGGER = 6        # fires on the 6th consecutive frame
#     iris_streak_direction = None
#     iris_streak_count = 0
...
#                 # ── Light / Brightness Monitoring (40% - 70%) ─────────────
#                 face_pct, bg_pct = analyze_lighting_conditions(frame, face_box)
...
#                         # ── Liveliness (No-Blink Warning, strict 8s) ──
#                         time_since_last_blink = current_timestamp - last_blink_timestamp
#                         if time_since_last_blink >= NO_BLINK_WARN_SECONDS:
#                             cv2.putText(frame, "WARN: PERSON NOT BLINKING (>8s)", (20, 290), font, 0.9, (0, 0, 255), 3)
#                             cv2.rectangle(frame, (0, 0), (width, height), (0, 0, 255), 4)
...
#                                 snap_path = snapshot_dir / f"no_blink_violation_{int(current_timestamp)}.jpg"
#                                 cv2.imwrite(str(snap_path), frame)
#                                 print(f"🚨 Candidate does not blink his eye! Snapshot: {snap_path}")
#                                 last_no_blink_snap_time = current_timestamp
#                         else:
#                             cv2.putText(frame, f"No Blink: {time_since_last_blink:.1f}s / {NO_BLINK_WARN_SECONDS:.0f}s", (20, 290), font, 0.6, (0, 255, 255), 2)

#                         # ── Iris / Gaze Direction Tracking ──
#                         if ear >= EYE_OPEN_FOR_GAZE_EAR:
#                             iris_dir, r_iris_pt, l_iris_pt = detect_iris_direction(face_landmarks.landmark, width, height)
#                             cv2.circle(frame, r_iris_pt, 2, (0, 255, 0), -1)
#                             cv2.circle(frame, l_iris_pt, 2, (0, 255, 0), -1)
...
#                                 if iris_streak_count == IRIS_STREAK_TRIGGER: # 6th consecutive frame
#                                     short_dir = iris_dir.replace("IRIS ", "").lower()

#                                     hr_text = f"looking {short_dir} side repeadliy 6 time"
#                                     ai_warning = f"please focused on screen don't look {short_dir}"

#                                     write_trigger_row(trigger_csv_path, user_id, formatted_time, hr_text, ai_warning)
#                                     print(f"🚨 [CSV LOGGED @ {formatted_time}] HR: '{hr_text}' | AI Warning: '{ai_warning}'")
...
# def write_trigger_row(csv_path, user_id: str, timestamp_ms: str,
#                       hr_text: str, ai_warning: str) -> None:
#     """Append one evidence row to the trigger-point CSV."""
#     csv_path = Path(csv_path)
#     ensure_trigger_csv_header(csv_path)
#     with open(csv_path, "a", newline="", encoding="utf-8") as f:
#         writer = csv.writer(f)
#         writer.writerow([user_id, timestamp_ms, hr_text, ai_warning])

# def side_label(short_dir: str) -> str:
#     labels = {"LEFT": "Left", "RIGHT": "Right", "UP": "Up", "DOWN": "Down"}
#     return labels.get(short_dir, short_dir)

# def side_adverb(short_dir: str) -> str:
#     adverbs = {"LEFT": "leftward", "RIGHT": "rightward",
#                "UP": "upward", "DOWN": "downward"}
#     return adverbs.get(short_dir, short_dir.lower())
...
#     video_dir = Path("video_interview")
#     snapshot_dir = video_dir / "snapshots"
#     trigger_dir = Path("trigger_points")
#     video_dir.mkdir(parents=True, exist_ok=True)
#     snapshot_dir.mkdir(parents=True, exist_ok=True)
#     trigger_dir.mkdir(parents=True, exist_ok=True)

#     # 1b. One unique ID per user, saved in the trigger CSV (stored in video_interview/)
#     user_id = f"USER_{uuid.uuid4().hex[:6].upper()}"
#     trigger_csv_path = video_dir / "trigger_points.csv"
#     ensure_trigger_csv_header(trigger_csv_path)
#     # Register the user's single unique ID in the CSV right at session start
#     write_trigger_row(trigger_csv_path, user_id, "0.00ms",
#                       f"User session started (Candidate: {candidate_name})",
#                       "No violation detected yet")
...
#     # ── Iris/Gaze debounce settings (FIX: prevents blink frames from
#     #    being misread as "IRIS DOWN", and requires a sustained gaze
#     #    shift before warning) ──────────────────────────────────────
#     EYE_OPEN_FOR_GAZE_EAR = 0.23   # must be clearly above blink threshold (0.18)
#     REQUIRED_IRIS_FRAMES = 4       # consecutive agreeing frames needed to warn
#     iris_direction_counter = 0
#     last_iris_direction = None

#     # ── Trigger Point (strict eye movement) state: fires on the 7th
#     #    consecutive confirmed look at the same side (>6 rule) ────────
...
#                     eye_closed_start_time = None
#                     last_blink_timestamp = current_timestamp
#                     # Reset iris debounce state when the candidate leaves frame
#                     iris_direction_counter = 0
#                     last_iris_direction = None
#                     iris_streak_direction = None
#                     iris_streak_count = 0
...
#                         # EAR is computed once here and reused below for both
#                         # the eye-closure check and the iris/gaze gating (FIX).
#                         ear = calculate_ear(face_landmarks.landmark, width, height)
...
#                         # ── Iris Direction Tracking (FIX APPLIED HERE) ───────
#                         # Only trust gaze direction when eyes are clearly open.
#                         # During a blink the iris landmark gets pinned near the
#                         # bottom eyelid, which used to fire a false "IRIS DOWN"
#                         # warning on every single blink. We also require several
#                         # consecutive frames to agree before warning, so one
#                         # noisy frame can't trigger a false positive.
#                         if ear >= EYE_OPEN_FOR_GAZE_EAR:
#                             iris_dir, r_iris_pt, l_iris_pt = detect_iris_direction(face_landmarks.landmark, width, height)
#                             cv2.circle(frame, r_iris_pt, 2, (0, 255, 0), -1)
#                             cv2.circle(frame, l_iris_pt, 2, (0, 255, 0), -1)

#                             if iris_dir is not None and iris_dir == last_iris_direction:
#                                 iris_direction_counter += 1
#                             else:
#                                 iris_direction_counter = 1
#                                 last_iris_direction = iris_dir

#                             if iris_dir and iris_direction_counter >= REQUIRED_IRIS_FRAMES:
...
#                                 # ── Trigger Point: >6 consecutive looks (7th fires) ──
#                                 short_dir = iris_dir.replace("IRIS ", "")
#                                 relative_ts = current_timestamp - start_time

#                                 if iris_dir == iris_streak_direction:
#                                     iris_streak_count += 1
#                                     gaze_timestamps.append(relative_ts)
#                                 else:
#                                     iris_streak_direction = iris_dir
#                                     iris_streak_count = 1
#                                     gaze_timestamps = [relative_ts]

#                                 if iris_streak_count > IRIS_STREAK_TRIGGER:
#                                     cv2.putText(frame, f"WARNING: EYE CONTINUOUSLY {short_dir} (x{iris_streak_count})", (20, 170), font, 1, (0, 0, 255), 3)
#                                     cv2.rectangle(frame, (0, 0), (width, height), (0, 0, 255), 4)
#                                     if current_timestamp - last_trigger_snap_time > 3.0:
#                                         # Snapshot trigger marker at the exact crossing timestamp
#                                         snap_path = trigger_dir / f"trigger_point_{short_dir}_{int(current_timestamp)}.jpg"
#                                         cv2.imwrite(str(snap_path), frame)

#                                         # Timestamp log sequence (session-relative milliseconds)
#                                         crossing_ms = relative_ts * 1000.0
#                                         seq_ts = [f"{t * 1000.0:.2f}ms" for t in gaze_timestamps[-GAZE_EVENT_LOG_MAX:]]
#                                         seq_repr = ", ".join(seq_ts)
#                                         if len(gaze_timestamps) > GAZE_EVENT_LOG_MAX:
#                                             seq_repr += "..."

#                                         hr_text = (f"User continuously looked at {side_label(short_dir)} Side "
#                                                    f"{iris_streak_count} times at timestamps [{seq_repr}]")
#                                         ai_warning = (f"WARNING: Repeated {side_adverb(short_dir)} gaze detected "
#                                                       f"({iris_streak_count} times). Possible off-screen source inspection. Snapshot captured.")

#                                         write_trigger_row(trigger_csv_path, user_id, f"{crossing_ms:.2f}ms", hr_text, ai_warning)
#                                         print(f"🚨 TRIGGER POINT ({user_id}): {hr_text}")
#                                         print(f"   {ai_warning}")
#                                         print(f"   Snapshot: {snap_path}")
#                                         last_trigger_snap_time = current_timestamp
#                         else:
#                             # Eyes closed / mid-blink — never let this count as a gaze shift
#                             iris_direction_counter = 0
#                             last_iris_direction = None
#                             iris_streak_direction = None
#                             iris_streak_count = 0
